...
def get_dataset(tfrecords,
    ...
    def parse_func(example_proto):
        feature_dict = {
            ...
            'subsegment/features': tf.FixedLenFeature([FEAT_SIZE[0] * FEAT_SIZE[1]], tf.float32),
    ...
    def _input_fn():
        ...
        if subsample:
            signal_input = tf.strided_slice(
                signal_input, begin=[0, 0, 0], end=[batch_size, input_size, 1], strides=[1, 8, 1]
            )
    ...
    try:
        ...
        while True:
            ...
            for i in range(len(signal_input)):
                ...
                sns.set()
                sns.lineplot(x=[i for i in range(len(signal_input[i]))], y=signal_input[i])
