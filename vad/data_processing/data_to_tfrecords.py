...
flags.DEFINE_integer(name='seq_len', default=1024, help='segment length')
flags.DEFINE_integer(name='num_shards', default=256, help='number of tfrecord files')
flags.DEFINE_boolean(name='debug', default=False, help='debug for a few samples')
flags.DEFINE_string(name='data_type', default='trainvaltest', help='data types to write into tfrecords')
...
def create_tfrecords(data_dir,
    ...
    if 'train' in data_type:
        ...
        if not tf.io.gfile.isdir(train_path):
    ...
    if 'val' in data_type:
        ...
        if not tf.io.gfile.isdir(val_path):
            tf.io.gfile.makedirs(val_path)
